...
let getUserInfo = function (username, socket) {
    return new Promise(function (resolve, reject) {
        UserDao.findByUsername(username).then(user=>{
            ...
            return MultiUserService.cacheUserInfo(username, socket);
        }).then(result=>{
            resolve(result);
        }).catch(err=>{
            console.log(err);
...
let markUserPosition = function(username, mapId, xAxis, yAxis, monsterType, socket){
    return new Promise(function (resolve, reject) {
        getUserInfo(username, socket).then(result=>{
            return UserDao.setPositon(username, mapId, xAxis, yAxis);
        }).then(result=>{
            if (monsterType >= 0){
                return FightService.generateFight(username, monsterType, socket);
            }else{
                return null;
            }
        }).then(fightInfo=>{
...
exports.getExistUserInfo = getExistUserInfo;